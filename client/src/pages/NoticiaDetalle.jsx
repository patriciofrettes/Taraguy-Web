...
// 1. URL CORRECTA (Sin repetir https)
const API_URL = "https://taraguyrugbyclub-hhgkcrevcgerf7bg.centralus-01.azurewebsites.net";

const NoticiaDetalle = () => {
    const { id } = useParams();
    const [noticia, setNoticia] = useState(null);
    const [error, setError] = useState(false); // Para mostrar error si falla la carga

    useEffect(() => {
        // 2. Usamos API_URL para no equivocarnos
    ...
    // 3. FUNCIÓN DE IMAGEN SIMPLIFICADA
    const getImagen = (ruta) => {
        if (!ruta) return "/img/default_news.png"; // Imagen local por defecto
        if (ruta.startsWith("http")) return ruta;
        // Concatenamos directo, funciona para /img y /uploads
        return `${API_URL}${ruta}`;
    };

    if (error) return <div className="text-center mt-20 text-red-600 font-bold">Error al cargar la noticia. Verifica tu conexión.</div>;
        ...
        <div className="min-h-screen bg-white pb-20">
            ...
            <div className="w-full h-96 overflow-hidden relative bg-black">
                <img
                    ...
                    // 4. QUITAMOS EL PLACEHOLDER EXTERNO QUE TE DABA ERROR
