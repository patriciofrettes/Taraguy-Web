...
const Partidos = () => {
    ...
    // 🔧 LÓGICA TIPO TIENDA (SIMPLE Y EFECTIVA)
    const getEscudo = (ruta) => {
        if (!ruta) return null;
        if (ruta.startsWith("http")) return ruta;
        // Concatenamos directo, igual que en la Tienda
        return `${API_URL}${ruta}`;
    };
                ...
                <div className="lg:col-span-2 bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div className="grid grid-cols-7 gap-2">
                        ...
                        {diasArray.map(dia => {
                            ...
                            return (
                                <div key={dia} className={`h-24 border rounded-lg p-2 flex flex-col justify-between transition hover:border-black ${esHoy ? 'bg-yellow-50 border-yellow-400' : 'bg-gray-50 border-gray-100'} ${hayPartido ? 'ring-2 ring-black bg-white' : ''}`}>
                                    <span className={`font-bold ${esHoy ? 'text-yellow-600' : 'text-gray-400'}`}>{dia}</span>
                                    <div className="flex flex-col gap-1 overflow-hidden">
                                        {partidosDia.map(p => <div key={p.id} className="text-[10px] bg-black text-white px-1 rounded truncate font-bold" title={p.rival}>vs {p.rival}</div>)}
                                    </div>
                                </div>
                            );
                        })}
                                ...
                                return (
                                    <div key={p.id} className="border-l-4 border-black pl-4 py-2">
                                        ...
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            {/* ESCUDO CON PROTECCIÓN DE ERROR */}
                                            {escudoUrl ? (
                                                <img
                                                    src={escudoUrl}
                                                    className="w-8 h-8 object-contain"
                                                    alt={p.rival}
                                                    // Si la imagen falla, la ocultamos y mostramos el div de abajo
                                                    onError={(e) => {
                                                        e.target.style.display = 'none';
                                                        e.target.nextSibling.style.display = 'flex'; // Truco: mostrar el siguiente elemento
                                                    }}
                                                />
                                            ) : null}

                                            {/* Este div se muestra si no hay URL o si la imagen dio error y la ocultamos */}
                                            <div
                                                className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center font-bold text-xs"
                                                style={{ display: escudoUrl ? 'none' : 'flex' }} // Oculto por defecto si hay URL
                                            >
                                                ?
                                            </div>
                                        </div>
